...
export const generateVideoBasicData = async (aimodel = "gemini-2.5-flash") => {
    ...
    try {
        ...
        const res = await fetch(
            `${BASE_URL}/totalCharacters.json?updatedAt=${Date.now()}`
        );
        ...
        const genre = Genre[Math.floor(Math.random() * Genre.length)];
        ...
        Choose ONE person and prepare the story so it can be turned into a highly engaging 20-30 seconds cinematic narration.
        It must feel like a Netflix-level documentary condensed into 20-30 seconds.
        ...
        DO NOT create new scripts or any other data for any of these characters:
        ...
        - For this video, please select a character that is specifically '${characterGender}'
        - And also you can select character from this field '${genre}' But if you have any better character idea from any better field you can go for it
        ...
        according to the work done by the person
        
        ========== GENDER ===========
        
        One word only.
        it can be either "male" or "female" not both or any other word just "male" or "female"
        according to the gender of our character
        ...
        Length: 20-30 seconds narration.
        
        The pacing must feel fast and cinematic.
        
        CRITICAL RULE
        The person's name must NOT be revealed until the final sentence.
        ...
        The opening line must immediately stop viewers from scrolling.
        
        The hook must feel shocking, mysterious, or emotionally intense.
        
        Use one of the following psychological triggers:
        
        • a shocking or disturbing fact
        • an impossible or unbelievable event
        • a hidden truth people never heard
        • a dangerous decision with huge consequences
        • a secret that changed history
        • a moral dilemma or controversial action
        
        STRICT RULES:
        - Maximum 12 words.
        - One or two sentences only.
        - No greetings.
        - Do NOT reveal the person's name.
        ...
        The final sentence must reveal the person's full name.
        
        The reveal must feel powerful, surprising, and emotionally satisfying.
        It should land as the dramatic conclusion of the story.
        ...
        Sentences must be punchy and dramatic.
        Use cinematic storytelling language.
        
        Every sentence must build curiosity, tension, or emotion.
        ...
        // ✅ Fix: Validate all required keys before uploading
        const requiredKeys = ["title", "desc", "tags", "name", "field", "script", "type", "gender", "musicstyle"];
        for (const key of requiredKeys) {
            if (!json[key]) {
                throw new Error(`Missing or empty required key in AI response: "${key}"`);
            }
        }

        // ✅ Fix: Validate type and gender values
        if (!["good", "evil"].includes(json.type)) {
            throw new Error(`Invalid "type" value: "${json.type}". Must be "good" or "evil".`);
        }

        if (!["male", "female"].includes(json.gender)) {
            throw new Error(`Invalid "gender" value: "${json.gender}". Must be "male" or "female".`);
        }
        ...
        console.log("📤 basicData.json uploaded successfully");
