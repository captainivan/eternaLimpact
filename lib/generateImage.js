...
const ai = new GoogleGenAI({ apiKey: process.env.GEMINI_API_KEY_SECOND });
...
const pollForImage = async (inferenceId, apiKey, maxAttempts = 300) => {
...
// Returns null if the key is exhausted/failed so the caller can switch keys.
const generateOneImage = async (promptObj, apiKey) => {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    // Treat missing inference_id as a hard key failure (e.g. credits exhausted)
...
export const generateImage = async () => {
  try {
    ...
    const imagePrompts = await fetch(
      `https://ik.imagekit.io/shunya/imagePrompts.json?updatedAt=${Date.now()}`
    );
    ...
    const imageArray = [];
    let currentKeyIndex = 0; // Start with the first key and only advance on failure

    for (let j = 0; j < prompts.length; j++) {
      let uploaded = null;

      // Try the current key; if it fails, advance to the next one
      while (currentKeyIndex < PICSART_API_KEYS.length) {
        const apiKey = PICSART_API_KEYS[currentKeyIndex];
        console.log(`\nProcessing prompt ${j + 1}/${prompts.length} (API key #${currentKeyIndex + 1})...`);

        try {
          uploaded = await generateOneImage(prompts[j], apiKey);

          if (uploaded?.url) {
            // Key worked — keep using it for the next prompt
            break;
          } else {
            // Key returned no result → credits low / exhausted → switch key
            console.warn(`  ⚠️  API key #${currentKeyIndex + 1} exhausted or failed. Switching to next key...`);
            currentKeyIndex++;
          }
        } catch (err) {
          console.error(`  Error with API key #${currentKeyIndex + 1}:`, err);
          currentKeyIndex++;
        }
      }

      if (currentKeyIndex >= PICSART_API_KEYS.length) {
        console.error("  🚫 All API keys exhausted. Stopping early.");
        break;
      }
    ...
    return {
      ...
      lastUsedKey: `PICSART_API_KEY_${currentKeyIndex + 1}`,
      totalKeys: PICSART_API_KEYS.length,
    };
  } catch (error) {
